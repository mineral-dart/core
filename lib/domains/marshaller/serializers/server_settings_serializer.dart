...
final class ServerSettingsSerializer implements SerializerContract<ServerSettings> {
  ...
  Map<String, dynamic> deserialize(ServerSettings object) {
    return {
      ...
      'explicit_content_filter': object.explicitContentFilter.toString().split('.').last,
      'verification_level': object.verificationLevel.toString().split('.').last,
      'default_message_notifications': object.defaultMessageNotifications.toString().split('.').last,
      'features': object.features,
      'mfa_level': object.mfaLevel.toString().split('.').last,
