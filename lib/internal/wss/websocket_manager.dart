...
final class WebsocketManager {
  ...
  WebsocketManager({ required this.logger, required this.http, required this.token, required this.intents, required this.dispatcher });
  ...
  Future _getBotGateway () async => Either.future(
    ...
    onError: (error) {
      logger.severe('Error while getting the bot gateway: ${error.message}');

      switch (error.statusCode) {
        case 401: throw Exception('Your token is invalid');
        default: throw Exception('(${error.statusCode}) ${error.message}');
      }
    }
  );
