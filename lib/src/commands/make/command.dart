...
class MakeCommand extends CliCommandContract {
  MakeCommand(ConsoleService console)
      : super(console, 'make:command', 'Create new command file', ['name']);

  @override
  Future<void> handle(Map args) async {
    final placeInRootFolder = prompts.getBool(
        'Would you like to change the location of your ?',
        defaultsTo: false);

    if (!placeInRootFolder) {
      final file = File(join(Directory.current.path, 'src',
          '${ReCase(args['name']).snakeCase}.dart'));
      _createFile(file, args);
      return;
    }

    final location =
        prompts.get('Where do you want to create your command file ?');
    final file = File(join(Directory.current.path, 'src', location,
        '${ReCase(args['name']).snakeCase}.dart'));
