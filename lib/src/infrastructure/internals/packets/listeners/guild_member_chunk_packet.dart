...
final class GuildMemberChunkPacket implements ListenablePacket {
  ...
  Future<void> listen(ShardMessage message, DispatchEvent dispatch) async {
    ...
    final presences = List<Map<String, dynamic>>.from(message.payload['presences'])
        .map(Presence.fromJson)
        .toList();

    final resolver =
        _wss.requestQueue.firstWhereOrNull((element) => element.uid == message.payload['nonce']);
    if (resolver != null && !resolver.completer.isCompleted) {
      if (resolver.targetKeys.length == 1 && resolver.targetKeys.contains('presences')) {
        resolver.completer.complete(presences.first);
      }

      if (resolver.targetKeys.length == 1 && resolver.targetKeys.contains('members')) {
        resolver.completer.complete(presences.first);
      }

      if (resolver.targetKeys.contains('members') && resolver.targetKeys.contains('presences')) {
        resolver.completer.complete({'members': members, 'presences': presences});
      }
